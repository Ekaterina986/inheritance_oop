...
def grade_average(list_students, course):
	summa = 0
	count = 0
	...
	return f'Средний бал по курсу: "{course}": {summa / count}'

# лекторы

def grade_average_lect(list_lecturers, course):
	summa = 0
	count = 0
	for lecturer in list_lecturers:
		if course in lecturer.courses_attached:
			res = sum(lecturer.grades_lecturer[course]) / len(lecturer.grades_lecturer[course])
			summa += res
			count += 1
	if count == 0:
		return'Нет такого курса'
	return f'Средний бал за лекции по курсу: "{course}": {summa / count}'
...
best_reviewer = Reviewer('Buddy', 'Some')
best_reviewer.courses_attached += ['JS']
...
best_reviewer.rate_student(best_student, 'Python', 10)
best_reviewer.rate_student(best_student, 'C++', 6)
best_reviewer.rate_student(best_student, 'Python', 8)
...
print(cool_reviewer)
print(best_reviewer)
print(best_student.count_average())
print(worst_student.count_average())
print(best_student > worst_student)
print(best_lecturer.count_average())
print(cool_lecturer.count_average())
print(cool_lecturer > best_lecturer)
print(*list_students, sep = ', ')
print(worst_student.grades)
print(grade_average(list_students, 'Python'))
print(grade_average_lect(list_lecturers, 'JS'))
print(best_lecturer.grades_lecturer)